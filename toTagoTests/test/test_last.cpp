...
#define ID_MQTT "650f53cc4d9a6300095323d4" // id mqtt (para identificar sesión)
...
void SerializeAndPublish(){
  ...
  doc_0["variable"] = "BeeHive1_Temp";
  doc_0["value"] = getTemperature();
  doc_0["unit"] = "C";

  JsonObject doc_1 = doc.createNestedObject();
  doc_1["variable"] = "BeeHive1_Hum";
  doc_1["value"] = getHumidity();
  doc_1["unit"] = "%";
  ...
  mqtt.publish(TopicsToPublish[0], buffer);

}
*/

/* Test Json variable by variable */
void SendDataToTago(String tipoDato){
  JsonDocument doc;
  char buffer[1024];
  JsonObject doc_0 = doc.createNestedObject();
  ...
  else if (tipoDato == "hum") {
    doc_0["variable"] = "BeeHive1_Hum";
    doc_0["value"] = getHumidity();
    //doc_0["unit"] = "%";
    char* topico = "Hum";
    serializeJson(doc, buffer);
    serializeJsonPretty(doc, Serial);
